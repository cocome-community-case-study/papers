...
Therefore, \CoCoME is encapsulated in a single component\todosk{ist die kapselung eine konsequenz das es nicht angepasst werden musste?}. 
...
Additionally, the \textit{AppShopAdatper} provides a \deleted{Rest}REST \deleted{Api}Interface which is used by the \deleted{actual }\textit{AppShop}. 
The \deleted{Rest}REST Interface\deleted{Api contains }provides endpoints to retrieve and process Credit Card, Enterprise and StockItem information. 
...
Maven is needed to \added{build and} deploy the latest version of \CoCoME onto the provided Glassfish servers\todosk{nur deployen?}.

During the development\added{ phase}\deleted{, it was}\added{ we} decided to implement and provide two different versions. 
The first version always pulls the most recent \CoCoME source code from GitHub, downloads the entire dependencies with Maven, compiles and builds the project and finally deploys \CoCoME on the Glassfish servers\todosk{gibt es mehere Glassfish Server?}. 
As a consequence, creating and starting a Docker Container takes about one hour\todosk{zusammenhang zum docker container unklar}.

In contrast, the second version \deleted{only }pulls a prefabricated version of \CoCoME from GitHub. 
Therefore, pulling the source code \deleted{up }to build\deleted{ing} the project is skipped. 
...
\added{Nevertheless,} the prefabricated version has to be updated manually\todosk{was bedeutet das genau?}. 
Thus, it is sometimes not the most recent version.

By providing both, a fast deploying version and a current version, the user can choose what\deleted{'s}\added{ suits best for his/her} situation.
